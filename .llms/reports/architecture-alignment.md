---
title: "brasa-skills-cli Architecture Alignment"
date: "2026-05-02"
...
`brasa-skills-cli` deve ser tratado como **repositorio de suporte organizacional** dentro da organizacao. O pacote npm detectado e `@brasalabs/skills`, com scripts principais: build, check, check:release, lint, pack:check, pack:dry-run, start, test, typecheck. Diretorios de primeiro nivel relevantes: `docs`, `src`, `catalog`, `node_modules`, `tests`, `dist`, `scripts`. Documentos/contratos detectados: `README.md`, `AGENTS.md`, `SPECs.md`, `package.json`.
...
- `catalog`: diretorio de primeiro nivel presente no checkout.
- `node_modules`: diretorio de primeiro nivel presente no checkout.
- `tests`: diretorio de primeiro nivel presente no checkout.
- `dist`: diretorio de primeiro nivel presente no checkout.
- `scripts`: diretorio de primeiro nivel presente no checkout.

## Build, Execucao E Validacao

Superficie de validacao inferida: `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, `pnpm start`.

## Relacoes Arquiteturais Imediatas

- `agents-specs`: relacao inferida por nome, dependencias, familia de produto ou stack.
...
- `skills`: relacao inferida por nome, dependencias, familia de produto ou stack.
- `specs`: relacao inferida por nome, dependencias, familia de produto ou stack.
